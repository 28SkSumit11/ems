...
class DataTableController extends Controller
{
    public function index($form_id = 5){
        ...
        $data = DB::table('wp_fluentform_entry_details')->where('form_id', $form_id)->limit(5000)->get();
        $arrTable = [];
        ...
        $data = DB::table('wp_fluentform_entry_details')
            ->where('form_id', $form_id)
            ->get()
            ->groupBy('submission_id');

        $formattedData = [];
        foreach ($data as $submission_id => $entries) {
            
            $name = '';
            $datetime = '';
            foreach ($entries as $entry) {
                if($entry->field_name == 'names'){
                    $name = $entry->field_value;
                }

                if($entry->field_name == 'datetime'){
                    $datetime = $entry->field_value;
                }
            }
            $formattedData[] = [
                'id' => $submission_id,
                'name' => $name,
                'datetime' => $datetime,
                'action' => '<i class="fa-solid fa-eye"></i>'
            ];
        }
        ...
        // dd($dj);
        return view('index', ['data' => $res, 'form_id' => $form_id]);
